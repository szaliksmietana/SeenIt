...
import { moviesApi, watchlistApi, reviewsApi, type Movie, type Review } from '../api/services';
...
async function saveReview() {
  ...
  try {
    if (myReview.value) {
      await reviewsApi.update(myReview.value.id, { rating: form.rating, content: form.content });
      toast.success('Recenzja zaktualizowana');
    } else {
      await reviewsApi.create({ movie_id: id, rating: form.rating, content: form.content });
...
<template>
  ...
  <p v-else-if="!movie" class="text-center text-secondary py-5">Nie znaleziono filmu.</p>

  <div v-else style="max-width: 700px">
    <RouterLink to="/movies" class="text-secondary d-inline-block mb-3">← Powrót do listy</RouterLink>
    ...
    <div class="d-flex gap-4 mb-4">
      ...
      <div>
        ...
        <p v-if="movie.avg_rating" class="fs-5 fw-bold text-warning mb-0">★ {{ movie.avg_rating }} / 10</p>
    ...
    <table class="table table-sm w-auto mb-4">
      <tbody>
        <tr><th class="text-secondary fw-normal pe-4">Reżyser</th><td>{{ movie.director || '—' }}</td></tr>
        <tr><th class="text-secondary fw-normal pe-4">Gatunek</th><td>{{ movie.genre || '—' }}</td></tr>
        <tr><th class="text-secondary fw-normal pe-4">Kraj</th><td>{{ movie.country || '—' }}</td></tr>
      </tbody>
    </table>
    ...
    <div class="d-flex gap-2 flex-wrap mb-4">
      <button @click="handleAddToWatchlist" class="btn btn-outline-secondary">+ Do obejrzenia</button>
      <template v-if="user?.role === 'admin'">
        <RouterLink :to="`/movies/${movie.id}/edit`" class="btn btn-outline-secondary">Edytuj</RouterLink>
        <button @click="handleDelete" class="btn btn-outline-danger">Usuń</button>
      </template>
    </div>
    ...
    <div class="card mb-4">
      <div class="card-body">
        <h3 class="h6 mb-3">{{ myReview ? 'Twoja recenzja' : 'Oceń ten film' }}</h3>
        ...
        <div class="mb-3">
          ...
          <textarea v-model="form.content" class="form-control" rows="3" placeholder="Co o nim sądzisz?"></textarea>
        </div>
        <div class="d-flex gap-2">
          <button @click="saveReview" class="btn btn-primary" :disabled="savingReview">
            {{ savingReview ? 'Zapisywanie...' : (myReview ? 'Zaktualizuj' : 'Dodaj recenzję') }}
          </button>
          <button v-if="myReview" @click="deleteReview" class="btn btn-outline-danger">Usuń recenzję</button>
        </div>
      </div>
    </div>

    <p v-if="reviews.length === 0" class="text-center text-secondary py-4">Brak recenzji. Bądź pierwszy!</p>
